#!/usr/bin/env node
// nos-comment-item: append a plain-string comment to an item's meta.yml > comments.
...
async function main() {
  ...
  if (typeof args.text !== 'string') die('missing_args', '--text <string> is required.');
  if (!args.text.trim()) die('empty_comment', '--text must be non-empty after trim.');
