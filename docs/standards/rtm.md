...
| REQ-00094 | Add mechanism to restart a workflow item | Feature request | `docs/standards/api-reference.md`, `docs/standards/error-handling-strategy.md`, `docs/standards/ux-design.md` | `app/api/workflows/[id]/items/[itemId]/restart/route.ts`, `lib/workflow-store.ts` (`restartItem`), `lib/activity-log.ts` (`restart` type), `components/dashboard/ItemDetailDialog.tsx` (Restart button + confirmation dialog), `lib/hooks/use-workflow-items.ts` (`restartItem` hook) | Manual validation — all 8 acceptance criteria verified | Done |
...
| REQ-00099 | Reorder sidemenu | Feature request | `docs/standards/ux-design.md` (§ Navigation), `docs/standards/wbs.md` (1.4.1) | `components/dashboard/Sidebar.tsx` | Visual regression check — sidebar items visible in correct order; Workflows collapsible; Activity navigation works | Done |
